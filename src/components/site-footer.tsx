import Link from "next/link";
import { BrandLogo } from "@/components/brand-logo";
import { ChangeCountryButton } from "@/components/change-country-button";

export function SiteFooter({
  email,
  whatsapp,
}: {
  email?: string;
  whatsapp?: string;
}) {
  return (
    <footer className="mt-auto border-t border-border bg-teal text-cream dark:bg-[#061012]">
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:px-6 sm:py-12 md:grid-cols-3">
        <div>
          <BrandLogo href="/home" size="md" variant="onDark" />
          <p className="mt-4 max-w-sm text-sm text-cream/75">
            Faith-guided education and modern IT — Holy Quran tutors, online tuition, and technology services worldwide.
          </p>
        </div>
        <div>
          <p className="text-sm font-semibold uppercase tracking-wider text-gold">Explore</p>
          <ul className="mt-3 space-y-2 text-sm text-cream/80">
            <li><Link href="/quran" className="hover:text-white">Holy Quran Tutors</Link></li>
            <li><Link href="/tuition" className="hover:text-white">Online Tuition</Link></li>
            <li><Link href="/it" className="hover:text-white">IT Services</Link></li>
            <li><Link href="/blog" className="hover:text-white">Blog</Link></li>
            <li><Link href="/pricing" className="hover:text-white">Pricing</Link></li>
          </ul>
        </div>
        <div>
          <p className="text-sm font-semibold uppercase tracking-wider text-gold">Contact</p>
          <ul className="mt-3 space-y-2 text-sm text-cream/80">
            {email && <li><a href={`mailto:${email}`} className="hover:text-white">{email}</a></li>}
            {whatsapp && (
              <li>
                <a
                  href={`https://wa.me/${whatsapp.replace(/\D/g, "")}`}
                  target="_blank"
                  rel="noreferrer"
                  className="hover:text-white"
                >
                  WhatsApp
                </a>
              </li>
            )}
            <li>
              <ChangeCountryButton label="Change country" variant="footer" />
            </li>
          </ul>
        </div>
      </div>
      <div className="border-t border-white/10 py-4 text-center text-xs text-cream/60">
        © {new Date().getFullYear()} Al-Hadi Institute. All rights reserved.
      </div>
    </footer>
  );
}
